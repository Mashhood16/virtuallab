...
import { ArrowLeft, Cpu, Zap, CheckCircle, XCircle, Power, ShieldAlert } from 'lucide-react';
    ...
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 font-sans select-none">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col items-center">
          ...
          <div className="w-full flex justify-center gap-4 mb-6">
            {(['AND', 'OR', 'XOR', 'NAND'] as const).map(gate => (
              <button
                ...
                className={`px-4 py-2 rounded-md font-semibold transition-colors ${gateType === gate ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
              >
                {gate}
              </button>
            ))}
          ...
          <div className="relative w-full max-w-md aspect-video bg-slate-900 rounded-xl shadow-inner p-4 flex items-center justify-between border-4 border-slate-700">
            <div className="flex flex-col gap-12 z-10">
              <button 
                onClick={() => setInputA(!inputA)}
                className={`w-12 h-12 rounded-full border-4 flex items-center justify-center transition-all ${inputA ? 'bg-green-500 border-green-300 shadow-[0_0_15px_rgba(34,197,94,0.6)]' : 'bg-slate-600 border-slate-500'}`}
              >
                <Power size={20} className={inputA ? 'text-white' : 'text-slate-400'} />
              </button>
              <button 
                onClick={() => setInputB(!inputB)}
                className={`w-12 h-12 rounded-full border-4 flex items-center justify-center transition-all ${inputB ? 'bg-green-500 border-green-300 shadow-[0_0_15px_rgba(34,197,94,0.6)]' : 'bg-slate-600 border-slate-500'}`}
              >
            ...
            <div className="z-10">
              <div className={`w-16 h-16 rounded-full border-4 transition-all flex items-center justify-center ${output ? 'bg-red-500 border-red-300 shadow-[0_0_30px_rgba(239,68,68,0.8)]' : 'bg-slate-700 border-slate-600'}`}>
                <span className={`font-bold text-xl ${output ? 'text-white' : 'text-slate-500'}`}>{output ? '1' : '0'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 text-center text-slate-600">
            ...
            <div className="flex justify-center gap-4 mt-2">
              <span className="px-3 py-1 bg-slate-200 rounded text-sm">Input A: {inputA ? '1' : '0'}</span>
              <span className="px-3 py-1 bg-slate-200 rounded text-sm">Input B: {inputB ? '1' : '0'}</span>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col">
          ...
          <div className="flex-1">
            ...
            <table className="w-full text-left border-collapse mb-6">
              <thead>
                <tr className="bg-slate-100">
                  <th className="p-2 border border-slate-200">A</th>
                  <th className="p-2 border border-slate-200">B</th>
                  <th className="p-2 border border-slate-200">Output</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td className="p-2 border border-slate-200">0</td>
                  <td className="p-2 border border-slate-200">0</td>
                  <td className="p-2 border border-slate-200">0</td>
                </tr>
                <tr>
                  <td className="p-2 border border-slate-200">0</td>
                  <td className="p-2 border border-slate-200">1</td>
                  <td className="p-2 border border-slate-200 text-indigo-600 font-bold">1</td>
                </tr>
                <tr>
                  <td className="p-2 border border-slate-200">1</td>
                  <td className="p-2 border border-slate-200">0</td>
            ...
            <div className="mb-4">
              <label className="block text-sm font-semibold text-slate-700 mb-2">Which logic gate does this system use?</label>
              <input
                type="text"
                value={assessmentAns}
