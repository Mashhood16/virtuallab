                ...
                <div className="bg-slate-50 p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col">
                    ...
                    {tab === 'cooling' && (
                        <div className="flex-grow flex flex-col">
                            <svg viewBox="0 0 400 300" className="w-full bg-slate-50 rounded-lg shadow-inner border mb-4">
                            ...
                            <div className="flex space-x-2 mb-4">
                                ...
                                <button onClick={() => { setRunC(false); setTimeC(0); }} className="flex-1 bg-slate-100 text-slate-700 py-2 rounded-md font-medium flex items-center justify-center hover:bg-slate-200">
                                    <RotateCcw className="w-4 h-4 mr-2" /> Reset
                                </button>
                            </div>
                            <label className="text-sm font-medium text-slate-700 mb-1">Cooling Constant ($k$): {kCool.toFixed(3)}</label>
                            <input type="range" min="0.01" max="0.1" step="0.01" value={kCool} onChange={(e) => setKCool(Number(e.target.value))} className="mb-2" />
                            <label className="text-sm font-medium text-slate-700 mb-1">Initial Temp ($T_0$): {t0}°C</label>
                            <input type="range" min="50" max="200" step="10" value={t0} onChange={(e) => setT0(Number(e.target.value))} className="mb-2" />
                            <label className="text-sm font-medium text-slate-700 mb-1">Ambient Temp ({"$T_{env}$"}): {tEnv}°C</label>
                    ...
                    {tab === 'ballistics' && (
                        <div className="flex-grow flex flex-col">
                            ...
                            <div className="flex space-x-2 mb-4">
                                ...
                                <button onClick={() => { setRunB(false); setTimeB(0); }} className="flex-1 bg-slate-100 text-slate-700 py-2 rounded-md font-medium flex items-center justify-center hover:bg-slate-200">
                                    <RotateCcw className="w-4 h-4 mr-2" /> Reset
                                </button>
                            </div>
                            <label className="text-sm font-medium text-slate-700 mb-1">Drag Coefficient ($k$): {drag.toFixed(2)} kg/s</label>
                            ...
                            <label className="text-sm font-medium text-slate-700 mb-1">Launch Angle: {angle}°</label>
