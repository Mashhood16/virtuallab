...
import { ArrowLeft, Flame, FlaskConical, RotateCcw, Check } from 'lucide-react';
...
export default function LabC10SaltTitration({ onExit }: { onExit?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 font-sans select-none">
      <header className="bg-orange-600 text-white p-4 shadow-md flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={onExit} className="p-2 hover:bg-orange-700 rounded-full transition-colors"><ArrowLeft size={24} /></button>
          <h1 className="text-2xl font-bold">Preparation of a Soluble Salt</h1>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col gap-6">
            <div className="bg-orange-50 p-5 rounded-xl border border-orange-200 mt-auto">
                ...
                <div className="flex gap-2">
                    <input type="text" value={assessmentAns} onChange={(e) => setAssessmentAns(e.target.value)} placeholder="Process name..." className="flex-1 px-3 py-2 border border-orange-300 rounded focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white" />
