...
export default function LabC11OrganicAnalysis({ onExit }: Props) {
  ...
  const getTubeColor = () => {
    ...
    if (obs.includes('Silver mirror')) return 'bg-slate-300 border-4 border-slate-400';
    if (obs.includes('Orange precipitate')) return 'bg-orange-500';
    if (obs.includes('Yellow precipitate')) return 'bg-yellow-300';
    if (obs.includes('Cloudiness')) return 'bg-slate-200 opacity-80';
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 p-4 min-h-0">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col items-center overflow-y-auto">
          ...
          <div className="relative w-32 h-64 mb-8 shrink-0">
            ...
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-36 h-4 bg-slate-200 rounded-full opacity-50 blur-sm"></div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
            {(['Bromine Water', 'Tollens', '2,4-DNPH', 'Iodoform', 'Lucas'] as Reagent[]).map((reagent) => (
              <button
                ...
                className={`flex flex-col items-center justify-center p-2 rounded border text-xs font-medium transition-colors ${activeReagent === reagent ? 'bg-purple-100 border-purple-500 text-purple-700' : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100'}`}
              >
          ...
          <button onClick={() => setActiveReagent('None')} className="mt-4 text-xs text-slate-500 hover:text-slate-700 flex items-center gap-1 shrink-0">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col overflow-y-auto">
          ...
          <div className="bg-slate-50 rounded-lg border border-slate-200 p-3 mb-4 min-h-[150px] overflow-y-auto">
          ...
          <div className="mt-auto pt-4 border-t border-slate-200 shrink-0">
            <h3 className="font-bold text-slate-800 mb-2">Identify the Unknown</h3>
            <p className="text-sm text-slate-600 mb-3">Based on your observations, what functional group class is present?</p>
            
            <select 
              value={selectedType}
              onChange={(e) => setSelectedType(e.target.value)}
              className="w-full p-2 mb-3 border border-slate-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
