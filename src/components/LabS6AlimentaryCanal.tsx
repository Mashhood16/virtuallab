...
export default function LabS6AlimentaryCanal({ onExit }: LabProps) {
  ...
  return (
    <div className="flex flex-col h-screen bg-rose-50 dark:bg-slate-900 font-sans">
      ...
      <div className="flex-1 flex flex-col p-8 items-center overflow-y-auto">
        <div className="w-full max-w-4xl bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8">
          ...
          <div className="flex gap-8">
            {/* Bank of organs */}
            <div className="w-1/3 bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
                 ...
                 {[...organs].sort().map(organ => {
                   ...
                   return (
                     <button
                       ...
                       disabled={isUsed}
                       className={`p-3 text-left font-bold border-2 rounded-lg transition-colors ${isUsed ? 'bg-slate-100 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-600 cursor-not-allowed' : 'bg-white dark:bg-slate-800 border-rose-200 dark:border-rose-800 text-rose-700 dark:text-rose-300 hover:border-rose-400 dark:hover:border-rose-600 hover:bg-rose-50 dark:hover:bg-slate-700 shadow-sm'}`}
                     >
                       {organ}
                     </button>
                   );
            ...
            <div className="flex-1 bg-slate-50 dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
               <div className="flex items-center justify-between mb-4">
                 ...
                 <button onClick={() => setOrder([])} className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 font-bold">Clear All</button>
               </div>
               
               <div className="flex flex-col gap-2">
                 {order.length === 0 && (
                   <div className="p-8 text-center text-slate-400 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl">
                     Select an organ from the left to start the pathway.
                   </div>
                 )}
                 {order.map((organ, index) => (
                   <div key={organ} className="flex items-center gap-4">
                     <div className="w-8 h-8 rounded-full bg-slate-800 dark:bg-slate-700 text-white flex items-center justify-center font-bold text-sm shrink-0">
                       {index + 1}
                     </div>
                      <div className="flex-1 p-3 bg-white border-2 border-rose-400 text-rose-900 dark:bg-slate-800 dark:border-rose-600 dark:text-rose-100 font-bold rounded-lg flex justify-between items-center shadow-sm">
                       {organ}
