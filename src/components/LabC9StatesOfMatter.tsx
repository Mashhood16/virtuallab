...
import { ArrowLeft, Beaker, Play, ClipboardList, CheckCircle, Flame, Thermometer } from 'lucide-react';
...
export default function LabC9StatesOfMatter({ onExit }: Props) {
  ...
  return (
    <div className="flex flex-col h-screen overflow-y-auto bg-slate-50 font-sans select-none">
      <div className="flex items-center p-4 bg-blue-900 text-white shadow-md">
        <button onClick={onExit} className="mr-4 hover:bg-blue-800 p-2 rounded"><ArrowLeft size={24} /></button>
        <h1 className="text-2xl font-bold flex-grow">Grade 9 Chemistry: States of Matter & Solutions</h1>
        <div className="flex gap-2">
          <button onClick={() => { setActiveTab('sugar'); resetSim(); }} className={`px-4 py-2 rounded ${activeTab === 'sugar' ? 'bg-blue-700' : 'bg-blue-800'}`}>Solutions</button>
          <button onClick={() => { setActiveTab('cuso4'); resetSim(); }} className={`px-4 py-2 rounded ${activeTab === 'cuso4' ? 'bg-blue-700' : 'bg-blue-800'}`}>Hydrates</button>
        </div>
      </div>
            ...
            <div className="flex flex-col items-center w-full">
              ...
              <div className="relative w-48 h-64 border-4 border-gray-300 rounded-b-3xl rounded-t-sm flex items-end justify-center bg-blue-50 overflow-hidden shadow-inner">
                ...
                {(sugarAdded > sugarDissolved) && (
                  <div className="absolute bottom-0 w-full bg-white opacity-90 border-t border-gray-200" style={{ height: `${Math.min((sugarAdded - sugarDissolved)/2, 20)}%` }}></div>
                )}
